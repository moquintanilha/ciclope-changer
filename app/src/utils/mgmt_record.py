...
def update_record_alternate(vpn_name: str, requester: str, token: str):
    if vpn_name == 'conductor':
        ...
        payload = {
            'ttl': os.environ['CONDUCTOR_RECORD_TTL'],
            'record_type': os.environ['CONDUCTOR_RECORD_TYPE'],
            'description': os.environ['CONDUCTOR_DESCRIPTION'],
            ...
            'status': 'TO_UPDATE',
            'provider_name': os.environ['CONDUCTOR_PROVIDER_NAME'],
            'weight': os.environ['CONDUCTOR_RECORD_ON_TRAFFIC']
        }
        headers = {
...
def update_record_location(vpn_name: str, requester: str, token: str):
    if vpn_name == 'conductor':
        ...
        payload = {
            ...
            'status': 'TO_UPDATE',
            'provider_name': os.environ['CONDUCTOR_PROVIDER_NAME'],
            'weight': os.environ['CONDUCTOR_RECORD_OFF_TRAFFIC']
        }
        headers = {
            'x-tiger-token': 'Bearer ' + token
        }
